...
/*
 * 기존 세그먼트 트리에 최솟값과 최댓값을 구하는 메소드를 생성
 * 
 * 각 구각별로 최소값과 최대값을 구하기 위해서 각각의 트리를 만들어준다.
 * 
 * */
...
public class BJ2357_최솟값과최댓값 {
	...
	public static void main(String[] args) throws IOException {
		...
		SegmentTreeMax streeMax = new SegmentTreeMax(n);

		streeMin.init(num, 1, n, 1);
		streeMax.init(num, 1, n, 1);
		...
		for (int i = 0; i < m; i++) {
			...
			sb.append(streeMin.min(1,n,1,a,b) + " " + streeMax.max(1,n,1,a,b)+"\n");
	...
	private static class SegmentTreeMax {
		...
		//생성자
		public SegmentTreeMax(int arrSize) {
			//트리의 높이를 이용하여 배열의 크기 구하기
			int h = (int) Math.ceil(Math.log(arrSize) / Math.log(2));
			this.treeSize = (int) Math.pow(2, h + 1);
			tree = new long[treeSize];
		...
		public long init(long[] arr, int start, int end, int node) {
			...
			//좌측, 우측으로 나눠져서 최대값을 초기화해준다.
			int mid = (start + end) / 2;
			return tree[node] = Math.max(init(arr, start, mid, node * 2),init(arr, mid + 1, end, node * 2 + 1));
		}
