...
public class BJ16234_인구이동 {
	...
	private static int getMoveBFS() {
		while (true) {
			boolean isMove = false;
			visited = new boolean[n][n]; //*새로 순회할때마다 방문처리 초기화!!!
	...
	//4. 모든 방문이 끝나면 list에 담긴 값끼리 인구이동(* list.size() > 1) --> 인구조건에 맞는 값으로 변경
	private static void getChangePepole(int peopleSum) {
	...
	private static int getCountryCntBFS(int x, int y) {
